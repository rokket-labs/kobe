  ...
  return (
    <div>
      <div style={{ width: 500, margin: 'auto' }}>
        ...
        <p>
          Deposit or Withdraw your liquidity, try to game the <i>evil</i> Vendor.
        </p>
        <p>
          To <b>deposit</b>, input the ETH amount. We&apos;ll automagically take care of the Token amount and approval.
        </p>
        <p>
          To <b>withdraw</b>, input your liquidity amount. Be careful not to put more!
        </p>
      </div>
      <h2>TVL</h2>
      <h3>
        Total Liquidity:
        <TokenBalance balance={contractTotalLiquidity} /> | KOY:
        <TokenBalance balance={contractTokenBalance} /> | ETH:
        <TokenBalance balance={contractEthBalance} />
      </h3>
      <h3>
        Your Current Liquidity: <TokenBalance balance={lpPosition} />
      </h3>
      <Space>
        <div style={{ padding: 8, marginTop: 32, width: 300, margin: 'auto' }}>
          <Card title="Add Liquidity">
            <div style={{ padding: 8 }}>
              ...
              />
              Koywe Tokens to Deposit:{' '}
              <TokenBalance balance={depositAllowanceNeeded && ethers.utils.parseEther(depositAllowanceNeeded)} />
            </div>
              ...
              <div style={{ padding: 8 }}>
                <Button
                  ...
                  onClick={async () => {
                    setDepositing(true)
                    await tx(
                      writeContracts.KoyweToken.approve(
                        dexAddress,
                        depositAllowanceNeeded && ethers.utils.parseEther(depositAllowanceNeeded),
                      ),
                    )
                    setDepositing(false)
                  }}
        ...
        <div style={{ padding: 8, marginTop: 32, width: 300, margin: 'auto' }}>
          <Card title="Withdraw Liquidity">
            <div style={{ padding: 8 }}>
              ...
              />
              Koywe Tokens to Receive{' '}
              <TokenBalance
                balance={liquidityToWithdraw && ethers.utils.parseEther(getWihdrawToken(liquidityToWithdraw))}
              />
            </div>
            <div style={{ padding: 8 }}>
              <Button
                ...
                onClick={async () => {
                  setWithdrawing(true)
                  await tx(
                    writeContracts.Dex.withdraw(liquidityToWithdraw && ethers.utils.parseEther(liquidityToWithdraw)),
                  )
